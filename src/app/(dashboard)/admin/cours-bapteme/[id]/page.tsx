...
export default function CoursBaptemeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
    ...
    <>
            ...
            <div className="max-h-60 overflow-y-auto space-y-1 mb-4">
              {availableMembers.map(m => (
                <label key={m.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-neutral-50 cursor-pointer">
                  <input type="checkbox" checked={selectedMemberIds.includes(m.id)} onChange={e => setSel(prev => e.target.checked ? [...prev, m.id] : prev.filter(x => x !== m.id))}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={sessionTitle} onChange={e => setSessTitle(e.target.value)} className="w-full border border-neutral-200 rounded-xl px-4 h-10 text-sm focus:outline-none focus:border-brand-500" placeholder="Séance 1 — Introduction" />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-neutral-500 mb-1">Date *</label>
                <input type="datetime-local" value={sessionDate} onChange={e => setSessDate(e.target.value)} className="w-full border border-neutral-200 rounded-xl px-4 h-10 text-sm focus:outline-none focus:border-brand-500" />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-neutral-500 mb-1">Notes</label>
                <textarea value={sessionNotes} onChange={e => setSessNotes(e.target.value)} rows={2} className="w-full border border-neutral-200 rounded-xl px-4 py-2 text-sm resize-none focus:outline-none focus:border-brand-500" />
              </div>
            </div>
      ...
      <div className="max-w-4xl space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <Link href="/admin/cours-bapteme" className="flex items-center gap-2 text-neutral-500 hover:text-brand-600 text-sm font-medium">
