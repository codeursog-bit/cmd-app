...
function ShareButtons({ url, title, description }: { url: string; title: string; description: string }) {
  ...
  const text = `${title}\n${description}`
  const platforms = [
    // ✅ sharer.php (sans double slash) — fonctionne sur PC, mobile browser et FB app
    { href: `https://www.facebook.com/sharer.php?u=${enc(url)}`, color: 'bg-[#1877F2]', label: 'f' },
    { href: `https://wa.me/?text=${enc(text + '\n\n' + url)}`,          color: 'bg-[#25D366]', label: 'W' },
    { href: `https://t.me/share/url?url=${enc(url)}&text=${enc(title)}`,  color: 'bg-[#229ED9]', label: 'T' },
    { href: `https://twitter.com/intent/tweet?text=${enc(title)}&url=${enc(url)}`, color: 'bg-black', label: 'X' },
  ]
    ...
    <div className="min-h-screen bg-white">
      ...
      {event.videoUrl && (
        <div className="max-w-4xl mx-auto px-6 mt-8">
          {ytId ? (
            <div className="relative w-full rounded-2xl overflow-hidden shadow-xl" style={{ paddingTop: '56.25%' }}>
              <iframe src={`https://www.youtube.com/embed/${ytId}`} title={event.title}
                className="absolute inset-0 w-full h-full"
